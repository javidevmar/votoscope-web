...
export default function CongressElections() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (errElection || !electionRow) {
        setError(errElection?.message ?? "No se encontró la elección 2023-07");
        setLoading(false);
        return;
      }
      ...
      const parsed: PartyHemicycle[] =
        data?.map((row: HemicicloRow) => {
          ...
          const partido = Array.isArray(partidoRaw)
            ? partidoRaw[0]
            : partidoRaw;
          return {
  ...
  if (loading) return <div className="p-4">Loading hemiciclo...</div>;
